...
%  AMAUC Freelas — Plataforma de Freelancing para Serviços Rurais
...
\hypersetup{
  ...
  urlcolor   = ifcGreen,
  citecolor  = ifcDark,
  pdftitle   = {AMAUC Freelas — Pré-Projeto de Projeto Integrador},
...
\begin{titlepage}
  ...
  \begin{center}
    ...
    {\LARGE\bfseries\color{ifcDark} AMAUC Freelas}\\[0.35cm]
    {\normalsize\itshape\color{ifcDark}
...
da plataforma \textbf{AMAUC Freelas} (referida no contexto acadêmico também
...
oportunidade concreta de aplicação tecnológica. A plataforma \textit{AMAUC Freelas}
...
Projetar, desenvolver e entregar a plataforma \textbf{AMAUC Freelas}:
...
O projeto \textbf{AMAUC Freelas} justifica-se pela confluência de três fatores:
...
\begin{table}[H]
  \centering
  \caption{Stack tecnológica --- AMAUC Freelas (planejamento GSD, 2026)}
  \label{tab:stack}
  \begin{tabularx}{\textwidth}{%
...
\begin{table}[H]
  \centering
  \caption{Fluxo geral de uso --- AMAUC Freelas (V1, fechamento externo)}
  \label{tab:fluxo}
  \begin{tabularx}{\textwidth}{%
...
A plataforma \textbf{AMAUC Freelas} parte de uma necessidade concreta e

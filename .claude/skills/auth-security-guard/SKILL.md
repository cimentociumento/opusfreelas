...
description: Auditoria de segurança do Opus Freelas — Clerk, JWT, DEV_BYPASS_TOKEN, RLS do Supabase, ownership de demandas, validação de input, secrets. Use sempre que a mudança tocar autenticação, autorização, permissão, ou antes de fechar qualquer correção que envolva dados de usuário.
---
...
`DEV_BYPASS_TOKEN` é atalho de desenvolvimento. O middleware **deve** rejeitá-lo
quando `NODE_ENV=production`. Essa checagem nunca sai, por nenhum motivo.
...
- [ ] Bypass continua bloqueado em produção?
...
- Bypass token vaza para produção via env mal configurada no deploy?
...
## Classificação
Bypass de auth, IDOR, secret exposto ou bypass ativo em produção são **S1** —
pare tudo. Falta de rate limit ou validação fraca em fluxo core costuma ser S2.
